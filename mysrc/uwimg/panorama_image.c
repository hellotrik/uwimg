...
float l1_distance(float *a, float *b, int n)
{
	...
	float dis;
	int i;
	for(i=0,dis=0;i<n;++i)dis+=fabs(a[i]-b[i]);
    return dis;
}
...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for(j = 0; j < an; ++j){
		...
		float dis=l1_distance(a[j].data,b[bind].data,a[j].n),dis2;
		for(i=1;i<bn;++i){
			dis2=l1_distance(a[j].data,b[i].data,a[j].n);
			if(dis2<dis){
				dis=dis2;
				bind=i;
			}
        ...
        m[j].p = a[j].p;
        m[j].q = b[bind].p;
        m[j].distance = dis; // <- should be the smallest L1 distance!
    }
	...
	for(i=0;i<an;i++)if(!seen[m[i].bi]){
		seen[m[i].bi]=i+1;
	}
	qsort(seen,bn,sizeof(int),cmp);
	for(i=0;i<bn;++i)if(seen[i]){
		if(count!=seen[i]-1)m[count]=m[seen[i]-1];
		++count;
	}
	*mn =count;
    free(seen);
...
point project_point(matrix H, point p)
{
	...
	c.data[1]=p.y;
	c.data[2]=1;
	matrix d=matmul_single(H,c);
    // TODO: project point p with homography H.
    // Remember that homogeneous coordinates are equivalent up to scalar.
    // Have to divide by.... something...
    point q = make_point(d.data[0]/d.data[2],d.data[1]/d.data[2]);
	FM(c);FM(d);
    return q;
}
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
	...
	for (i = 0; i < n; ++i){
		m[i].distance = point_distance(project_point(H,m[i].p),m[i].q);
	}
	qsort(m, n, sizeof(match), match_compare);
	for (i = 0; i < n; ++i,++count)if (m[i].distance > thresh)break;	
    return count;
}
...
matrix compute_homography(match *matches, int n)
{
	...
	memcpy(H.data,a.data,sizeof(float)*a.rows*a.cols);
	H.data[8] = 1;
    FM(a);
    return H;
}
...
matrix RANSAC(match *m, int n, float thresh, int k, int cutoff)
{
	...
	for (e = 0; e < k; ++e) {
		...
		if(!H.cols)continue;
		tmp=model_inliers(H, m, n, thresh);
		if (best < tmp) {
			matrix H1 = compute_homography(m,tmp);
			if(H1.cols){
				FM(H);
				H=H1;
				tmp=model_inliers(H, m, n, thresh);
...
image combine_images(image a, image b, matrix H)
{
	...
	for (j = 0; j <h; ++j) {
		for (i = 0; i < w; ++i) {
			pp=project_point(H, make_point(i,j));
			pp.x+=dx;pp.y+=dy;
			if(pp.x>=0&&pp.x<b.w&&pp.y>=0&&pp.y<b.h)
			for (k = 0; k < b.c; ++k) {
				// TODO: fill in.				
				set_pixel(c,i,j, k, polate(b, pp.x,pp.y, k,0));
			}
		}
	}
...
image panorama_image(image a, image b, float sigma, float thresh, int nms, float inlier_thresh, int iters, int cutoff)
{
    ...
    image comb = combine_images(a, b, H);FM(H);
    return comb;
}
...
image cylindrical_project(image im, float f)
{
	...
	Unroll:
		...
		y'=f*Y'/Z'+yc
	*/
	int xc=im.w>>1,yc=im.h>>1;
	int i,j,k;
	int w = 2 * f * atan2f(xc, f) - 1;
    image c = make_image(w, im.h, im.c);
	for (j = -yc; j < c.h - yc; ++j)for (k = -w / 2; k <= w / 2; ++k){
		float theta = k / f;
		float h = j / f;
		float X = sinf(theta);
		float Z = cosf(theta);
		...
		for (i = 0; i < c.c; ++i)
		set_pixel(c, k+w/2, j+yc, i,polate(im, x, y, i,0));
	}
    return c;
}
